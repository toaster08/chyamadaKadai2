...
final class ViewController: UIViewController {
    ...
    override func viewDidLayoutSubviews() {
        operatorSegment.setTitle("+", forSegmentAt: 0)
        operatorSegment.setTitle("-", forSegmentAt: 1)
        operatorSegment.setTitle("×", forSegmentAt: 2)
        operatorSegment.setTitle("÷", forSegmentAt: 3)
    }
}
...
@objc extension ViewController {
    private func calculate() {
        ...
        var sum: Double
        let firstNumber = Double(firstField.text!) ?? 0
        print(firstNumber)
        let secondNumber = Double(secondField.text!) ?? 0
        print(secondNumber)
